...
class DolbySettingsFragment : PreferenceFragment(),
    ...
    private fun updateSpeakerState() {
        audioManager?.getDevicesForAttributes(ATTRIBUTES_MEDIA)?.firstOrNull()?.let { device ->
            isOnSpeaker = (device.type == AudioDeviceInfo.TYPE_BUILTIN_SPEAKER)
        } ?: run {
            dlog(TAG, "No audio device found")
            isOnSpeaker = false
        }
    }
    ...
    private fun updateProfileSpecificPrefs() {
        ...
        bassPref.apply {
            isChecked = dolbyController.getBassEnhancerEnabled(currentProfile)
            summary = null
        }
